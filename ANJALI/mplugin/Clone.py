...
from ANJALI import ANJALI as app, save_clonebot_owner
from ANJALI import db as mongodb, ANJALI
...
async def clone_txt(client, message):
    if len(message.command) > 1:
        ...
        try:
            ai = Client(bot_token, API_ID, API_HASH, bot_token=bot_token, plugins=dict(root="ANJALI/mplugin"))
            await ai.start()
            bot = await ai.get_me()
            bot_id = bot.id
        ...
        try:
            ...
            await mi.edit_text(f"**Your bot successfully cloned👍**\n**You can check by /cloned**\n\n**But please start me (@{ANJALI.username}) From owner id**")
        
        except BaseException as e:
            logging.exception("Error while cloning bot.")
            await mi.edit_text(
                f"⚠️ <b>Error:</b>\n\n<code>{e}</code>\n\n**Forward this message to @AnjaliOwnerBot for assistance**"
            )
    else:
